...
export function ContratoBuilder({
  ...
  async function handleSalvar(status: "ORCAMENTO" | "CONFIRMADO") {
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 22, alignItems: "start" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button className="btn btn-g" disabled={pendente} onClick={() => handleSalvar("ORCAMENTO")}>
            Salvar como orçamento
          </button>
          <button className="btn btn-p" disabled={pendente} onClick={() => handleSalvar("CONFIRMADO")}>
            {pendente ? "Salvando..." : "Confirmar contrato"}
          </button>
        </div>
      </div>
